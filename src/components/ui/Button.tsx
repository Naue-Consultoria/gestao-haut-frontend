...
export function Button({ variant = 'dark', size = 'md', icon, children, className = '', ...props }: ButtonProps) {
  ...
  const variants = {
    ...
    icon: 'bg-transparent border border-gray-200 rounded-sm text-gray-500 hover:border-gray-400 hover:text-black',
  };
  const sizes = {
    sm: variant === 'icon' ? 'w-9 h-9' : 'px-4 py-2 text-xs',
    md: variant === 'icon' ? 'w-11 h-11' : 'px-6 py-3',
  };
